...
@param {Object} settings For detailed information about each option check documentation.
  ...
  @param {Number} [settings.max_retries=0] How many times to retry the chunk or file, before triggering Error event.
	@param {Number} [settings.wait_before_retry=1000] How long to wait in milliseconds after a failed chunk before attempting thenext retry.
	@param {Number} [settings.resets_on_error=0] On uploading error, number of times to reset the retry counter and pause uploading
...
plupload.Uploader = function(options) {
    ...
    , resets = up.settings.resets_on_error
		, wait = up.settings.wait_before_retry
    ...
    function triggerError(eventName) {
      up.trigger(eventName, {
        ...
        if (resets > 0 && up.settings.reset_count++ < resets) {
          triggerError('ResetOnError');
        } else {
          up.settings.reset_count = 0;
          triggerError('Error');
        }
			}
		}
